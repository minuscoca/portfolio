...
export function Project({ project }: { project: Project }) {
  return (
    <div className="group grid w-full cursor-pointer grid-cols-1 overflow-hidden rounded-xl bg-rose-100 dark:bg-rose-800/50 sm:grid-cols-2">
      <div
        id="project_detail"
        className="relative flex min-h-[10rem] flex-col px-4 group-even:order-last"
      >
...
function ProjectStacks({ stacks }: { stacks: Stack[] }) {
  return (
    <ul className="flex w-full flex-wrap justify-center p-4">
      {stacks.map((stack) => (
        <Badge
          key={stack.key}
...
function ProjectHeader({ project }: { project: Project }) {
  return (
    <>
      <h1 className="mt-4 px-4 text-3xl font-extrabold">{project.name}</h1>
